...
parser.add_argument('--num-classes', type=int, default=17, metavar='N',
...
parser.add_argument('--model-ema', action='store_false', default=False,
...
parser.add_argument('--real-labels', default='./images/real.json', type=str, metavar='FILENAME',
...
def main():
    ...
    # if args.real_labels:
    #     print("use real labels")
    #     real_labels = RealLabelsImagenet(dataset_eval.filenames(basename=True), real_json=args.real_labels)
    # else:
    #     real_labels = None
    real_labels = None
    ...
    if args.eval_checkpoint:  # evaluate the model
        ...
        val_metrics = validate(model, loader_eval, validate_loss_fn, args, real_labels=real_labels)
        print(f"Top-1 accuracy of the model is: {val_metrics['top1']:.1f}%")
        print(f"Top-5 accuracy of the model is: {val_metrics['top5']:.1f}%")
        print(f"Top-1 real accuracy of the model is: {val_metrics['top1a']:.1f}%")
        print(f"Top-5 real accuracy of the model is: {val_metrics['top5a']:.1f}%")
        end_time = time.time()
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', real_labels=None):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if data_list[target] in sums:
              sums[data_list[target]]+=1
            else:
              sums[data_list[target]]=1
            print(output.shape)
            print("-------------")
            ...
            print(pred)
            print('--------',data_list[pred])
            if target == pred:
              print("success")
              if data_list[target] in counts:
                counts[data_list[target]]+=1
                counts1[data_list[target]]+=1
              else:
                counts[data_list[target]]=1    
                counts1[data_list[target]]=1      
    ...
    metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg), ('top5', top5_m.avg),
                            ('top1a', round(top1a, 4)), ('top5a', round(top5a, 4))])
    for key in counts:
      if key in sums:
          counts[key] = sums[key] -counts[key]
          counts1[key] /= sums[key]
    print(counts)
    print(counts1)
    return metrics
